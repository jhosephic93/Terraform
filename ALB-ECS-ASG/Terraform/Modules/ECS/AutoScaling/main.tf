...
resource "aws_appautoscaling_policy" "cpu" {
  name               = "ecs_scale_cpu_service_${var.ecs_service_name}"
...
resource "aws_appautoscaling_policy" "memory" {
  name               = "ecs_scale_memory_service_${var.ecs_service_name}"
  resource_id        = aws_appautoscaling_target.ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_target.service_namespace
  policy_type        = "TargetTrackingScaling"
...
resource "aws_cloudwatch_metric_alarm" "high-memory-policy-alarm" {
  alarm_name          = "high-memory-ecs-service-${var.ecs_service_name}"
  alarm_description   = "High Memory for ecs service-${var.ecs_service_name}"
...
resource "aws_cloudwatch_metric_alarm" "high-cpu-policy-alarm" {
  alarm_name          = "high-cpu-ecs-service-${var.ecs_service_name}"
  alarm_description   = "High CPUPolicy Landing Page for ecs service-${var.ecs_service_name}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
